Ext.define('Jacada.system.ui.cti.CTIBar', {
	    ...
	    initCtiStates: function(){
	    	this.states = Ext.create('Jacada.system.ui.cti.CTIStates');
	    },
		    ...
		    if($W().UserCTIRoles.CTIDialList){
		  		if(this.isEMCCTIProvider()){
				  	$W().dialListWindow = Ext.create('Ext.window.Window', {
	     	        	   ...
	     	        	   items: Ext.create('Jacada.system.ui.cti.DirectoryDialListPanel'),
	     	        	   listeners:{
				    			show: function(){
			  	   	...
			  	   	$W().dialListWindow = Ext.create('Jacada.system.ui.cti.DialList');
		  	   	}
		    }else{
	    ...
	    buildCustomSection: function(){
	    	return Ext.create('Jacada.system.ui.cti.SectionPanel', {
	    	...
	    	return {
					...
					listeners: {
						afterRender: function(){
							try{
								$W().directorySearchWindow = Ext.create('Jacada.system.ui.directorySearch.DirectorySearchWindow');
							}catch(ex){
		...
		buildNotificationsBar: function(){
			return Ext.create('Jacada.system.ui.cti.SectionPanel', {
	    		...
	    		items: Ext.create('Jacada.system.ui.cti.CTINotificationsBar')
	    	});
		},
	    ...
	    buildVoiceActionBar: function(){
	    	return Ext.create('Jacada.system.ui.cti.VoiceActionBar');
	    },
	    buildOpenMediaActionBar: function(){
	    	return Ext.create('Jacada.system.ui.cti.OpenMediaActionBar');
	    },
	    buildTasksActionBar: function(){
	    	return Ext.create('Jacada.system.ui.cti.TasksActionBar');
	    },
	    buildEmailActionBar: function(){
	    	return Ext.create('Jacada.system.ui.cti.EmailActionBar');
	    },
	    buildChatActionBar: function(){
	    	return Ext.create('Jacada.system.ui.cti.ChatActionBar');
	    },
	    ...
	    showState: function(ctiClientInfo) {
    		...
    		Jacada.Logger.info("showState: " + clientStateName + ", errorMsg: " + ctiClientInfo.errorMessage + ", callId: " + ctiClientInfo.callId);
	        if(this.ctiClientInfo != null){
	            ...
	            var prevCtiClientInfo = this.ctiClientInfo;
	        }
	         ...
	         if(this.ctiClientInfo != null && this.ctiClientInfo.clientStateName != "current"){
	            ...
	            this.prevCtiClientInfoNotCurrent = this.ctiClientInfo;
	            if(this.ctiClientInfo.callId){
	            	this.prevStateCallId = this.ctiClientInfo.callId;
	            }else{
	            	this.prevStateCallId = null;
	            }
	        }
	        this.ctiClientInfo = Object.clone(ctiClientInfo);
	        
	        //update VoiceNode if such exits
	        var voiceNode = null;
	        var contextNavigation = Ext.getCmp('contextNavigation');
			if (contextNavigation && clientStateName != 'current') {
				var groupId = null;
				if(this.prevStateCallId){
					groupId = contextNavigation.buildVoiceGroupId(this.prevStateCallId);
				}else{
					//could be a voice work item already open without a call and changing from ready/notready.
					groupId = this.findNoCallVoiceContext(ctiClientInfo);
				}	
				
				if(groupId){
		        	voiceNode = contextNavigation.findNode(groupId);
		        	/*
		        	 * In case there is a context open in disposition with internal call,
		        	 * don't update the context node uiState in disposition with the new state
		        	 */
		        	if(clientStateName.indexOf('disposition_ringing') != -1 ||
		        			clientStateName.indexOf('dialing_inbound') != -1){
		        		Jacada.Logger.debug("CTIBar.js 'disposition' " + clientStateName 
		        				+ ", voiceNode.callId: " + (voiceNode?voiceNode.callId:'NULL')
		        				+ ", ctiClientInfo.callId: " + ctiClientInfo.callId);
		        		if(voiceNode && voiceNode.callId != ctiClientInfo.callId){
			        		voiceNode = null;
		        		}
		        	}
		        	
		        	if(voiceNode){
		        		Jacada.Logger.debug("Found VoiceNode for gropuId: " + groupId);
			        	if(voiceNode.callExists){
				        	voiceNode.uiState = Object.clone(this.states[clientStateName]);
				        	voiceNode.uiStateName = clientStateName;
			   		     	voiceNode.ctiClientInfo = ctiClientInfo;
			   		     	if(ctiClientInfo.callId && (voiceNode.callId != ctiClientInfo.callId)){
			  		     		/*
			  		     		 * call Id changes to the call with the super uppon join to conference,
			  		     		 * So it is required to create secondaryItemId so it can be found correctly. 
			  		     		 */
			   		     		Jacada.Logger.debug("CTIBar.showState(), secondaryItemId for Call Id that changed from : " + voiceNode.callId + ", to: " + ctiClientInfo.callId);
			   		     		voiceNode.data.secondaryItemId =  contextNavigation.buildVoiceGroupId(ctiClientInfo.callId);
			   		     		voiceNode.callId = ctiClientInfo.callId;
			   		     	}
			        	}else if(!voiceNode.callExists && voiceNode.customerDropped){
			        		if(voiceNode.callId != ctiClientInfo.callId){
			        			/*
			        			 * Customer disconnected but the context is still open so keep it related to the internal call
			        			 */
			        			voiceNode.data.secondaryItemId =  contextNavigation.buildVoiceGroupId(ctiClientInfo.callId);
			        		}else{
				        		/*
				        		 * The customer dropped but the call with the super continues,
				        		 * so remove the secodary id to Not find the customer node with the super call id.
				        		 */			        			
			        			voiceNode.data.secondaryItemId = null;
			        		}
			        	}
	        		}else if(this.prevStateCallId != ctiClientInfo.callId){
	        			Jacada.Logger.debug("CTIBar.showState(), could not find voice node for callId "+ctiClientInfo.callId);
	        			//we could not find the node, let's check if we got new CallID, if so, let's save the new CallID
	        			//as the secondary call ID.
	        			var groupId = contextNavigation.buildVoiceGroupId(this.prevStateCallId);
	        			Jacada.Logger.debug("Going to search for node with itemId "+groupId);
	        			voiceNode = contextNavigation.findNode(groupId);
	        			if(voiceNode){
	        				voiceNode.data.secondaryItemId = contextNavigation.buildVoiceGroupId(ctiClientInfo.callId);
	        				Jacada.Logger.debug("Found node with itemId "+groupId+" , updating secondaryItemId ="+voiceNode.data.secondaryItemId);
	        			}
	        		}
	        ...
	        var readyBtn = Ext.getCmp('readyBtn');
	        ...
	        if (this.outboundCalls == true) {
	            busy = false;
	            busyBtn.canBeEnabled = false;
	        }
	        
	        if(clientStateName == "current" && ctiClientInfo.busyChanged){
	        	Jacada.Logger.debug("Seems that we got future agent state: busy?"+busy);
	        	//If busy then readyBtn is visible
	        	if(busy){
	        		if(!busyBtn.hasCls('future-ready-border')){
	        			busyBtn.addCls('future-ready-border');
	        		}
	        	}else{
	        		if(!readyBtn.hasCls('future-busy-border')){
	        			readyBtn.addCls('future-busy-border');
	        		}
	        	}
	        }
	        
	        // in ringing, disable both ready & busy buttons
	        if (ctiClientInfo.clientStateName == "inwork_dialing" ||
	            ctiClientInfo.clientStateName == "inwork_incall" ||
	            ctiClientInfo.clientStateName == "inwork_inhold")
	        {
	            this._setEnabledDisabled(readyBtn, false);
	            this._setEnabledDisabled(busyBtn, false);
	        }
	        
	        this._handleErrorMessage(this.ctiClientInfo);
	        
	        //settings the busy reason even if the state name hasn't changed
	        this._handleReasonCodeInfo(this.ctiClientInfo);
        	...
        	this.state = Object.clone(this.states[clientStateName]); //this is not a deep clone
	        this.stateName = clientStateName;

	        if (!this.state){        
	            Jacada.Logger.debug('unexpected state: ' + clientStateName);
	            return;
	        }    
	        
	        
	        if(voiceNode && voiceNode.callExists && ctiClientInfo.clientStateName == 'call_ended' && ctiClientInfo.callId){
	        	Jacada.Logger.debug('CTIBar.js.showState, set callExists=false to voiceNode with itemId: ' + voiceNode.data.itemId);
	        	voiceNode.callExists = false;
	        }
	        
	       	/*
	       	 * Close voice context handling:
	       	 * Check if required to move node uiState to close state
	       	 */
	        if(voiceNode){
	        	if(!voiceNode.callExists || (this.isGENESYSCTIProvider() && voiceNode.callExists && voiceNode.customerDropped)){
		        //set this voice node with no call voice cotext state
    			Jacada.Logger.debug('CTIBar.js.showState, set noCallState to voiceNode with itemId: ' + voiceNode.data.itemId);
	        	var noCallStateName = this.getNoCallStateName();
    		    voiceNode.uiState = Object.clone(this.states[noCallStateName]);
    		    voiceNode.uiStateName = noCallStateName;
	        	}
	        }

	       	/*
	       	 * Close voice context handling:
	       	 * Ignore other states except ready, notready and disposition states while there is no call voice context
	       	 */
	        if(voiceNode && voiceNode.callExists == false &&
	        		ctiClientInfo.clientStateName != 'ready' &&
	        		ctiClientInfo.clientStateName != 'notready' &&
	        		ctiClientInfo.clientStateName != 'disposition' &&
	        		ctiClientInfo.clientStateName != 'disposition_ringing' &&
	        		ctiClientInfo.clientStateName != 'dialing_inbound' &&
	        		ctiClientInfo.clientStateName != 'call_ended' &&
	        		!voiceNode.customerDropped){
	        	Jacada.Logger.debug('CTIBar.js Close voice context handling. ctiClientInfo.clientStateName:' 
	        			+ ctiClientInfo.clientStateName + '. Returning');
	        	return;
	        }

	       	/*
	       	 * Close voice context handling:
	       	 * Handle UI state for dispostion or other no call states
	       	 */
    		if(voiceNode){
    			 if(ctiClientInfo.clientStateName.indexOf('disposition') > -1){//in case there is disposition
	        		Jacada.Logger.debug("CTIBar.js 'disposition' " + ctiClientInfo.clientStateName 
	        				+ ", voiceNode.uiState: " + (voiceNode.uiState?voiceNode.uiStateName:'NULL'));
    				 
        			if(voiceNode.uiState){
	        			voiceNode.uiStateName = ctiClientInfo.clientStateName;
	        			voiceNode.uiState.status = this.state.status;
	        			voiceNode.uiState.answerBtn = this.state.answerBtn;
	        			voiceNode.uiState.closeContextBtn = { visible: true,    enabled: true};		
        			}
        		}else if((!voiceNode.callExists || (this.isGENESYSCTIProvider() && voiceNode.callExists && voiceNode.customerDropped))&&
        					(ctiClientInfo.clientStateName == 'ready' 
        						|| ctiClientInfo.clientStateName == 'notready' || voiceNode.customerDropped)){
					var noCallStateName = this.getNoCallStateName();		        							
					voiceNode.uiState = this.copyStateSettingsToNoCallStateSettings(this.state, Object.clone(this.states[noCallStateName]));
					voiceNode.uiStateName = noCallStateName;
        		}
    		}

	        Ext.getCmp('transferBtn').canBeEnabled = this.ctiClientInfo.hasTransferInfo;
	        Ext.getCmp('dialBtn').canBeEnabled = this.ctiClientInfo.hasDialInfo;
	        Ext.getCmp('consultBtn').canBeEnabled = this.ctiClientInfo.hasConsultInfo;
	        
	        var state = (voiceNode && voiceNode.uiState && $W().activeContext.groupId.indexOf($W().interactionType.General) == -1) ? voiceNode.uiState : this.state;
	        /*
	         * In case there is a voice node and current state is disposition,
	         * don't display the close button in general context.
	         */
	        if(voiceNode && voiceNode.uiState && ctiClientInfo.clientStateName.indexOf('disposition') > -1 && $W().activeContext.groupId.indexOf($W().interactionType.General) > -1){
	        	Jacada.Logger.debug("CTIBar.js 'disposition' active groupId: " + $W().activeContext.groupId);
	        	state.closeContextBtn = { visible: false,    enabled: false};	
	        }
	        
	        /*
	         * process state only if the current node Id is active or general,
	         * otherwise only update the status text
	         */
	        if(!$W().activeContext 
	        		|| voiceNode && ($W().activeContext.groupId == (voiceNode.data.itemId || voiceNode.data.secondaryItemId))
	        		|| $W().activeContext.groupId.indexOf($W().interactionType.General) > -1
	        		|| !voiceNode){
	        	this.processVoiceState(state, this.ctiClientInfo);
	        }else{
	            if(state.status != null) {
	        		this.changePhoneStatus(state.status.text, state.status.clss);
	        	}
	        }
	
	        //if the state is 'ready' or 'notready',
	        //we need to udpate ready/busy buttons from real state
	        //since they might be out of synch
	        if(ctiClientInfo.clientStateName == 'ready' || ctiClientInfo.clientStateName == 'notready'){
	        	state = {
	        		busyBtn: this.state.busyBtn,
	        		readyBtn: this.state.readyBtn
	        	};
	        	this.processVoiceState(state, this.ctiClientInfo);
	        }

	        //Special case for end buttons when LayoutByContext is true
	        //and agent handles work item and a call at the same time
	        if(ctiClientInfo.clientStateName == "inwork_incall" 
	        	||ctiClientInfo.clientStateName == "inwork_inhold"){
	        	
	        	if($W().LayoutByContext){
	        		var endBtn = Ext.getCmp('endBtn');
	        		var endOpenMediaBtn = Ext.getCmp('endOpenMediaBtn');
	        		endBtn.show();
	        		endOpenMediaBtn.show();
	        	}
	        }
...
//	    	}  
	        ...
	        $W().handleCTILoginLogoutLink(this.ctiClientInfo.isDisplayCTILoginLinks,
	        		this.ctiClientInfo.isInSemiManual, stateAllowLogout);
//	        
