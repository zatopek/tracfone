...
ToolMan._dragFactory = {
	...
	_isDebuggingOnBrowser: function() {
		return false;
	},
...
_ToolManDragGroup.prototype = {
	...
	onTopWhileDragging : function(zIndex) {
		...
		// Support for IE8 - changed the attribute name from z-index to zIndex:
	...
	_dragEnd : function(event) {
		...
		event = ToolMan.events().fix(event)
		if( this.toolManDragGroup != undefined )
        {   
            var group = this.toolManDragGroup
            this.bkpGroup = group
        }else{
         if( this.bkpGroup != undefined )
            group = this.bkpGroup
         else
            return;
        }
